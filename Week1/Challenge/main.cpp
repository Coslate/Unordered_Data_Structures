...
inline int hashFunc(int value){
    int hashed_value = value % 1000;
    return hashed_value;
}
...
int insert(int value, std::vector<int> &table) {
  ...
  // trying to insert value into table.
    ...
    int hashed_value = hashFunc(value);
    int count_collision = 0;
    int step = 1;

    while(table[hashed_value] != -1){
        ++count_collision;
        hashed_value = hashFunc(hashed_value+step);
    }

    table[hashed_value] = value;
